...
const DEFAULT_FORWARDED_AI_BOT_JID = '867051314767696@bot'

/**
 * Build a contextInfo object for botForwardedMessage payloads.
 * @param {object|null} quoted - optional quoted WAMessage
 * @param {string} [botJid=DEFAULT_FORWARDED_AI_BOT_JID] - bot identity for forwarded AI metadata
 */
const buildRichContextInfo = (quoted, botJid = DEFAULT_FORWARDED_AI_BOT_JID) => {
    const ctxInfo = {
        forwardingScore: 1,
        isForwarded: true,
 ...
 */
const generateLatexImageContent = async (quoted, options, renderLatexToPng, uploadFn) => {
    const { text, expressions, headerText, footer } = options
    const submessages = []

    let latexRenderer = renderLatexToPng
    let mediaUploader = uploadFn

    if (expressions.length > 0) {
        const probeResult = await renderLatexToPng(expressions[0].latexExpression)
        const looksLikeRenderResult =
            probeResult &&
            typeof probeResult === 'object' &&
            'buffer' in probeResult &&
            'width' in probeResult &&
            'height' in probeResult

        if (!looksLikeRenderResult) {
            mediaUploader = renderLatexToPng
            latexRenderer = uploadFn
        }
    }
    ...
    const latexExpressions = await Promise.all(
        expressions.map(async expr => {
            const { buffer, width, height } = await latexRenderer(expr.latexExpression)
            const uploadResult = await mediaUploader(buffer, 'image')
