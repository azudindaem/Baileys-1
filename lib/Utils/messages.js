...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        // Token types: KEYWORD, STRING, NUMBER, METHOD, COMMENT, DEFAULT (from built-in tokenizer).
        const { text, code, language = 'javascript', botJid } = message.richResponse

        if (!botJid) {
            throw new boom_1.Boom('richResponse.botJid is required')
        }
        const sections = [
            {
                view_model: {
